...
function showBasicResourceProduction(t, i) {
    this.__blS.hasOwnProperty(i) && this.__blS[i].cont.setVisibility(t ? u : n)
}

function showOffenseRepairProduction(t) {
    this.__blT.setVisibility(t ? u : n)
}

function update(n) {
    var i, t;
    if (n != null) {
        this.__blT.isVisible() && this.__blX(n);
        for (i in this.__blS) t = this.__blS[i], t.cont.isVisible() && this.__blW(n, t)
    }
}