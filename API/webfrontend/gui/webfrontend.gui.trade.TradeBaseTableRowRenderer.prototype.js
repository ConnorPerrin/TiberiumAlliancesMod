...
function getHighlightFocusRow anonymous() {
    this.getHighlightFocusRow.$$install && this.getHighlightFocusRow.$$install();
    return this.getHighlightFocusRow.apply(this, arguments);
}

function setHighlightFocusRow anonymous() {
    this.setHighlightFocusRow.$$install && this.setHighlightFocusRow.$$install.call(this);
    return this.setHighlightFocusRow.apply(this, arguments);
}

function resetHighlightFocusRow() {
    return qx.core.Property.executeOptimizedSetter(this, n, i, b)
}

function setRuntimeHighlightFocusRow() {
    return qx.core.Property.executeOptimizedSetter(this, n, i, o, arguments)
}

function resetRuntimeHighlightFocusRow() {
    return qx.core.Property.executeOptimizedSetter(this, n, i, g)
}

function toggleHighlightFocusRow anonymous() {
    return this.setHighlightFocusRow(!this.getHighlightFocusRow())
}

function isHighlightFocusRow anonymous() {
    return this.getHighlightFocusRow()
}

function _renderFont(t) {
    t ? (this.__bpi = t.getStyles(), this.__bph = qx.bom.element.Style.compile(this.__bpi), this.__bph = this.__bph.replace(/"/g, e)) : (this.__bph = n, this.__bpi = qx.bom.Font.getDefaultStyles())
}

function updateDataRowElement(n, i) {
    var f = this.__bpi,
        u = i.style,
        r;
    qx.bom.element.Style.setStyles(i, f);
    r = ClientLib.Data.MainData.GetInstance().get_Cities().GetCity(n.rowData[t]);
    u.backgroundColor = r != null && (r.CanTrade() != ClientLib.Data.ETradeError.None || r.GetResourceCount(this.selectedResourceType) < 1) ? this.__bpj : n.focusedRow && this.getHighlightFocusRow() ? n.selected ? this._colors.bgcolFocusedSelected : this._colors.bgcolFocused : n.selected ? this._colors.bgcolSelected : n.row % 2 == 0 ? this._colors.bgcolEven : this._colors.bgcolOdd;
    u.color = n.selected ? this._colors.colSelected : this._colors.colNormal;
    u.borderBottom = a + this._colors.horLine
}

function getRowHeightStyle(n) {
    return qx.core.Environment.get(h) == r && (n -= this._insetY), f + n + d
}

function createRowStyle(i) {
    var r = [],
        u;
    return r.push(g), r.push(this.__bph), r.push(p), u = ClientLib.Data.MainData.GetInstance().get_Cities().GetCity(i.rowData[t]), u != null && (u.CanTrade() != ClientLib.Data.ETradeError.None || u.GetResourceCount(this.selectedResourceType) < 1) ? r.push(this.__bpj) : i.focusedRow && this.getHighlightFocusRow() ? r.push(i.selected ? this._colors.bgcolFocusedSelected : this._colors.bgcolFocused) : i.selected ? r.push(this._colors.bgcolSelected) : r.push(i.row % 2 == 0 ? this._colors.bgcolEven : this._colors.bgcolOdd), r.push(c), r.push(i.selected ? this._colors.colSelected : this._colors.colNormal), r.push(w, this._colors.horLine), r.join(n)
}
