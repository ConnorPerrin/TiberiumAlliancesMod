...
function update(n) {
    (this.__dS = n, !this.__dS || this.__dS.length <= 0) || (this.__Ka(), this.__Kb(), this.__Kd())
}

function updateRights(n, t) {
    this.__JX && !n && this.getVisibility() == p && (this.close(), webfrontend.gui.forum.ForumOverlay.getInstance().showInsufficientRightsMessage());
    this.__JX = n;
    this.__JY = t;
