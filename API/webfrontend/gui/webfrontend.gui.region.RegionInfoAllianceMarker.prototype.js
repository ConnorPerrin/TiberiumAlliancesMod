...
function setObject(n) {
    this.__RG = n;
    var f = ClientLib.Data.MainData.GetInstance().get_Player().get_Id(),
        t = this.__RG.get_Data(),
        u = !t.get_Description() || t.get_Description() == h ? e : t.get_Description();
    t.get_Description() == r && (u = r + this.tr(webfrontend.data.AllianceEvents.getInstance().getAllianceMarkerDescriptionTranslation(t.get_Type())));
    t.get_IdPlayerCreated() == f ? this.__RC.setSource(webfrontend.gui.alliance.AllianceMarkerNavs.getIconForMyType(t.get_Type())) : this.__RC.setSource(webfrontend.gui.alliance.AllianceMarkerNavs.getIconForType(t.get_Type()));
    this.__Rr.setValue(this.tr(webfrontend.data.AllianceEvents.getInstance().getAllianceMarkerTypeTranslation(t.get_Type())));
    this.__Ro.setPlayer(t.get_NamePlayerEdited(), t.get_IdPlayerEdited());
    this.__Rn.setPlayer(t.get_NamePlayerCreated(), t.get_IdPlayerCreated());
    this.__Rn.setVisibility(i);
    this.__Rp.setVisibility(i);
    this.__Rm.set({
        x: this.__RG.get_RawX(),
        y: this.__RG.get_RawY()
    });
    this.__RE.setValue(o + ClientLib.Data.MainData.GetInstance().get_World().SurroundedEnemies(this.__RG.get_RawX(), this.__RG.get_RawY()).toString() + p);
    this.__yY.set({
        value: u
    })
}
...
function startMoveMarkerOnRegion(n) {
    ClientLib.Vis.VisMain.GetInstance().GetMouseTool(ClientLib.Vis.MouseTool.EMouseTool.MoveMarker).IsReserveMarker = !1;
    this.__RI == null && (ClientLib.Vis.VisMain.GetInstance().SetMouseTool(ClientLib.Vis.MouseTool.EMouseTool.MoveMarker, n), qx.core.Init.getApplication().getBackgroundArea().closeCityInfo(), this.__RI = n || 0)
}

function startMoveMarkerOnRegionFromReserveBtn(n, t) {
    this.__QY = n;
    ClientLib.Vis.VisMain.GetInstance().GetMouseTool(ClientLib.Vis.MouseTool.EMouseTool.MoveMarker).IsReserveMarker = !0;
    ClientLib.Vis.VisMain.GetInstance().SetMouseTool(ClientLib.Vis.MouseTool.EMouseTool.MoveMarker, t);
    qx.core.Init.getApplication().getBackgroundArea().closeCityInfo();
    this.__RI = t || 0
}

function startRemoveMarkerOnRegion(i, r) {
    this.__QY = i;
    ClientLib.Vis.VisMain.GetInstance().SetMouseTool(ClientLib.Vis.MouseTool.EMouseTool.None, null);
    this.__RH = r;
    this.__RF.addListenerOnce(t, function(t) {
        t.getData() && (webfrontend.phe.cnc.Util.attachNetEvent(ClientLib.Data.MainData.GetInstance().get_Alliance(), n, ClientLib.Data.AllianceSetMarkerResult, this, this.__RB), ClientLib.Data.MainData.GetInstance().get_Alliance().DeleteMarker(this.__RH, this.__QY), this.__RH = null)
    }, this);
    this.__RF.askAboutRemove()
}
