...
function _onAppearFake() {
    this.__Od();
    this.__GE();
    this.__Os || (webfrontend.phe.cnc.Util.attachNetEvent(ClientLib.Data.MainData.GetInstance().get_Alliance(), o, ClientLib.Data.AllianceChange, this, this._onAllianceDetailChanged), webfrontend.phe.cnc.Util.attachNetEvent(ClientLib.Data.MainData.GetInstance().get_Alliance(), h, ClientLib.Data.AllianceCanModerateChange, this, this.__OM), this.__Os = !0)
}

function _onDisappearFake() {
    this.__Os && (webfrontend.phe.cnc.Util.detachNetEvent(ClientLib.Data.MainData.GetInstance().get_Alliance(), o, ClientLib.Data.AllianceChange, this, this._onAllianceDetailChanged), webfrontend.phe.cnc.Util.detachNetEvent(ClientLib.Data.MainData.GetInstance().get_Alliance(), h, ClientLib.Data.AllianceCanModerateChange, this, this.__OM), this.__Os = !1);
    this.__Pa()
}
...
function _onClickTableCell(n) {
    ...
    switch (n.getColumn()) {
        case 2:
            t = this.__OP.getRowData(n.getRow());
            webfrontend.gui.info.PlayerInfoWindow.getInstance().openWithPlayerId(t.InitiatingPlayerId);
            break;
        case 3:
            t = this.__OP.getRowData(n.getRow());
            webfrontend.gui.info.PlayerInfoWindow.getInstance().openWithPlayerId(t.PlayerId)
    }
}

function _onClickEditMoD() {
    this.__LC.setVisibility(n);
    this.__OX.setVisibility(t);
    this.__OY.setVisibility(t);
    this.__OU.setVisibility(n);
    this.__OW.setVisibility(t);
    this.__OR = this.__OW.getValue()
}
...
function _onClickOK() {
    ...
    this.__OU.setVisibility(t);
    this.__OW.setVisibility(n);
    ClientLib.Net.CommunicationManager.GetInstance().SendSimpleCommand(fi, {
        announcement: this.__OW.getValue()
    }, null, null)
}
...
function _onClickPreview() {
    ...
    this.__OU.setVisibility(t);
    this.__OW.setVisibility(n);
    this.__OS = this.__OV.getValue();
    this.__OV.setValue(l + webfrontend.gui.util.BBCode.convertBBCode(this.__OW.getValue()) + f)
}
...
function _onClickCancel() {
    ...
    this.__OU.setVisibility(t);
    this.__OW.setVisibility(n);
    this.__OW.setValue(this.__OR)
}

function _onKeyDownMoD() {
    var n = webfrontend.gui.alliance.NewsContainer.MaxMoDLength - this.form.__OW.getValue().length;
    this.lblCharCount.setValue(n.toString());
    n < 0 ? (this.btnOK.setEnabled(!1), this.lblAvailChars.setTextColor(s), this.lblCharCount.setTextColor(s)) : (this.btnOK.setEnabled(!0), this.lblAvailChars.setTextColor(r), this.lblCharCount.setTextColor(r))
}