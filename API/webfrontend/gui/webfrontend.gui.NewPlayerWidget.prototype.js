...
function executeCreateRandom() {
    this.create(8);
    this.__beV.setEnabled(!1)
}

function create(n) {
    if (!(this.__beD <= 0) && !(this.__beD > 3) && !(n < 0) && !(n > 8)) {
        var t = this.__beB[n].prefix;
        ClientLib.Net.CommunicationManager.GetInstance().SendSimpleCommand(hu, {
            name: this.__beN.getValue(),
            faction: this.__beD,
            startDir: t
        }, webfrontend.phe.cnc.Util.createEventDelegate(ClientLib.Net.CommandResult, this, function(n, t) {
            var i, r, u;
            this.__beV.setEnabled(!0);
            i = t;
            i == ClientLib.Base.EErrorCode.Success ? (r = qx.core.Init.getApplication(), r.removeNewPlayerCreationPages()) : i == ClientLib.Base.EErrorCode.Presignup ? webfrontend.gui.MessageBox.messageBox({
                title: this.tr(cr),
                text: this.tr(vr, webfrontend.phe.cnc.Util.getDateTimeString(new Date(this.__beC))),
                width: 400,
                textRich: !0,
                buttons: 1,
                executeOk: function() {
                    qx.core.Init.getApplication().redirectBrowser(LandingPageURL)
                },
                callbackContext: this
            }) : (u = this.tr(wi), alert(u), this.__bfq())
        }), null);
        this.__beD = ClientLib.Base.EFactionType.NotInitialized
    }
}

function dispose() {
    var n = qx.core.Init.getApplication();
    qx.event.Registration.removeListener(qx.dom.Node.getWindow(n.getRoot()), d, this._onDesktopResize, this)
}
...
function _onDesktopResize() {
    ...
    this.setLayoutProperties({
        top: Math.floor((t - this.getHeight()) / 2),
        left: Math.floor((n - this.getWidth()) / 2)
    })
}