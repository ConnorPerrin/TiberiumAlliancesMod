...
function getOffsetWidth() {
    return this.__bck && (this.__bcm = this.__in.offsetWidth), this.__bcm
}

function incrementTimesDisplayed() {
    var i, t;
    this.__bch.nodeValue = this.__Et.get_CommentsAllowed() ? this.__Et.get_NumComments() : r;
    this.__bcl == 0 && (i = new qx.util.StringBuilder(1024), i.add(u, this.__bco(), webfrontend.gui.notifications.TickerItem.TEXT_DIV_READ_STYLE), this.__bci.setAttribute(n, i.get()), this.__bcj.setAttribute(n, webfrontend.gui.notifications.TickerItem.END_DIV_READ_STYLE));
    t = new qx.util.StringBuilder(1024);
    t.add(webfrontend.gui.notifications.TickerItem.COMMENTS_NUM_DIV_STYLE);
    this.__Et.get_NumUnreadComments() > 0 && t.add(webfrontend.gui.notifications.TickerItem.BOLD_STYLE);
    this.__bcf.setAttribute(n, t.get());
    this.__bcl++;
    this.__bck = !0
}
...
function canDisplay() {
    var t = new Date,
        n = this.__Et.GetJSLastCommentTime();
    return n.setMinutes(n.getMinutes() + 5), t <= n && this.__Et.get_Reason() != ClientLib.Data.ENotificationReason.NotificationExpired && (this.__Et.get_TickerPasses() == 0 || this.__Et.get_TickerPasses() > this.__bcl + 1)
}

function updateCommentCount() {
    this.__bch.nodeValue = this.__Et.get_CommentsAllowed() ? this.__Et.get_NumComments() : r;
    this.__bcn(this.__Et);
    this.__Et.get_IsHot() ? this.__bcg.setAttribute(n, webfrontend.gui.notifications.TickerItem.HOT_TOPIC_HOT_STYLE) : this.__bcg.setAttribute(n, webfrontend.gui.notifications.TickerItem.HOT_TOPIC_NOT_STYLE);
    this.__bck = !0
}

function refresh() {
    if (this.__Et.GetJSLastCommentTime().getTime() >= this.__bce.getTime() + 6e4) {
        var t = new qx.util.StringBuilder(1024);
        t.add(u, this.__bco(), webfrontend.gui.notifications.TickerItem.TEXT_DIV_STYLE);
        this.__bci.setAttribute(n, t.get());
        this.__bcj.setAttribute(n, webfrontend.gui.notifications.TickerItem.END_DIV_STYLE);
        this.__bcl = 0;
        this.__bce = this.__Et.GetJSTime();
        this.__bck = !0
    }
}

function getDBId() {
    return this.__Et.get_Id()
}

function getIdOnlineOnly() {
    return this.__Et.get_IdOnlineOnly()
}

function _onDblClick() {
    var n = this.__Et.OpenPopup();
    n > 0
}

function _onNotificationHotnessUpdated() {
    this.__Et.get_IsHot() ? this.__bcg.setAttribute(n, webfrontend.gui.notifications.TickerItem.HOT_TOPIC_HOT_STYLE) : this.__bcg.setAttribute(n, webfrontend.gui.notifications.TickerItem.HOT_TOPIC_NOT_STYLE);
    this.__bck = !0
}