...
function _onAppearFake() {
    this.__Os || (webfrontend.phe.cnc.Util.attachNetEvent(ClientLib.Data.MainData.GetInstance().get_Player(), i, ClientLib.Data.PlayerChange, this, this._onInvitationsChanged), this.__Os = !0);
    ClientLib.Data.MainData.GetInstance().get_Player().RefreshMyInvitations()
}

function _onDisappearFake() {
    this.__Os && (webfrontend.phe.cnc.Util.detachNetEvent(ClientLib.Data.MainData.GetInstance().get_Player(), i, ClientLib.Data.PlayerChange, this, this._onInvitationsChanged), this.__Os = !1)
}
...
function _onClickAcceptInvitation() {
    var i = ClientLib.Data.MainData.GetInstance().get_Alliance(),
        n, t;
    i.get_Exists() ? (n = qx.core.Init.getApplication(), t = n.tr(nt), i.get_IsFirstLeader() && (t = n.tr(a)), webfrontend.gui.MessageBox.messageBox({
        title: n.tr(g),
        text: t,
        textRich: !0,
        buttons: 1,
        width: 400
    })) : ClientLib.Net.CommunicationManager.GetInstance().SendSimpleCommand(k, {
        invitationId: this.id,
        allianceId: this.allianceId
    }, webfrontend.phe.cnc.Util.createEventDelegate(ClientLib.Net.CommandResult, this.form, this.form._onAllianceInviteAccept), null)
}

function _onAllianceInviteAccept(n, t) {
    if (t != null)
        if (t.r != 0) {
            var i = qx.core.Init.getApplication(),
                r = i.tr(f);
            switch (t.r) {
                case ClientLib.Base.EErrorCode.AllianceWithoutAdmin:
                    r = i.tr(it);
                    break;
                case ClientLib.Base.EErrorCode.AllianceInvationAcceptFailedGracePeriodBlock:
                    r = i.tr(rt)
            }
        ...
        } else ClientLib.Data.MainData.GetInstance().get_Player().RefreshMyInvitations()
}

function _onClickDeclineInvitation() {
    ClientLib.Net.CommunicationManager.GetInstance().SendSimpleCommand(v, {
        invitationId: this.id,
        allianceId: this.allianceId
    }, webfrontend.phe.cnc.Util.createEventDelegate(ClientLib.Net.CommandResult, this.form, this.form._onAllianceInviteDecline), null)
}

function _onAllianceInviteDecline() {
    ClientLib.Data.MainData.GetInstance().get_Player().RefreshMyInvitations()
}