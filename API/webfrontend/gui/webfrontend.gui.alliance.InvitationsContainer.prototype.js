...
function triggerAppear() {
    this.__Os || (webfrontend.phe.cnc.Util.attachNetEvent(ClientLib.Data.MainData.GetInstance().get_Alliance(), f, ClientLib.Data.AllianceChange, this, this._onAllianceDetailChanged), this.__Os = !0);
    this.__Sw()
}

function _onDisappearFake() {
    this.__Os && (webfrontend.phe.cnc.Util.detachNetEvent(ClientLib.Data.MainData.GetInstance().get_Alliance(), f, ClientLib.Data.AllianceChange, this, this._onAllianceDetailChanged), this.__Os = !1)
}
...
function _onClickTableCell(n) {
    ...
    switch (n.getColumn()) {
        case 0:
            t = this.__Sq.getRowData(n.getRow());
            t.b = !t.b;
            this.__Sq.setValue(0, n.getRow(), t.b);
            this.__Sx();
            break;
        case 1:
            t = this.__Sq.getRowData(n.getRow());
            webfrontend.gui.info.PlayerInfoWindow.getInstance().openWithPlayerId(t.InviteeId);
            break;
        case 2:
            t = this.__Sq.getRowData(n.getRow());
            webfrontend.gui.info.AllianceInfoWindow.getInstance().openWithAllianceId(t.InviteeAllianceId)
    }
}

function _onSelectAllInvitations(n) {
    var t, r, i;
    for (this.__tp = [], t = this.__Sq.getData(), r = t.length, i = 0; i < r; i++) t[i] = t[i].originalData, t[i].b = n.getData(), t[i].b && this.__tp.push(t[i].Id);
    this.__Sq.setDataAsMapArray(t, !0);
    this.__Ss.setEnabled(this.__tp.length > 0)
}

function _onClickWithdrawInvitation() {
    ClientLib.Net.CommunicationManager.GetInstance().SendSimpleCommand(g, {
        invitationIds: this.__tp
    }, webfrontend.phe.cnc.Util.createEventDelegate(ClientLib.Net.CommandResult, this, this._onAllianceInviteRecall), null)
}

function _onAllianceInviteRecall(n, t) {
    if (t == null) {
    ...
    t != 0 ? webfrontend.gui.MessageBox.messageBox({
        title: this.tr(lt),
        text: this.tr(yt),
        buttons: 1
    }) : this.__St.setValue(!1)
}

function _onClickInvitePlayer() {
    webfrontend.gui.alliance.AllianceUtil.invitePlayerToAlliance(this.name.getValue(), !1, this.name)
}