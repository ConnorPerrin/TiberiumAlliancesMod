...
function _deferredAppear() {
    ...
    this._onClickResetRights();
    this._onAllianceDetailChanged()
}
...
function _onAllianceDetailChanged() {
    var t = ClientLib.Data.MainData.GetInstance().get_Alliance(),
        i, n;
    if (t.get_Id() != 0 && t.get_CanEditRights()) this.__Ty();
    else {
        for (i = this.__Tr.getChildren(), n = i.length - 1, n; n >= 0; n--) i[n].destroy();
        this.__Tv = [];
        this.__Tw = [];
        this.__Tu = [];
        this.close();
        t.get_CanEditRights() || webfrontend.gui.alliance.AllianceUtil.openLostRightsPopup()
    }
}
...
function _onClickUpdateRights() {
    for (var i, u = ClientLib.Data.MainData.GetInstance().get_Alliance().get_Roles().d, r = [], f = [], t = [], e = this.__Tv.length, o = this.__Tq.length + 2, n = 0; n < e; n++)
        for (r[n] = this.__Tv[n][0], f[n] = u[r[n]].IsDefaultName ? u[r[n]].Name : this.__Tv[n][1], t[n] = 0, i = 2; i < o; i++) this.__Tv[n][i].getValue() && (t[n] += 1 << this.__Tq[i - 2].right);
    t[0] += 1 << webfrontend.gui.alliance.MemberRightsWindow.ERoleCanTrackOutgoingAttacks;
    t[1] += 1 << webfrontend.gui.alliance.MemberRightsWindow.ERoleCanTrackOutgoingAttacks;
    ClientLib.Net.CommunicationManager.GetInstance().SendSimpleCommand(yi, {
        IDs: r,
        names: f,
        roles: t
    }, webfrontend.phe.cnc.Util.createEventDelegate(ClientLib.Net.CommandResult, this, this._onUpdateRightsDone), null);
    this.__Tt.setEnabled(!1)
}

function _onUpdateRightsDone(n, t) {
    t != 0 && this.__Tz(this.__Tu)
}

function _onClickResetRights() {
    this.__Tz(this.__Tu);
    this.__Tt.setEnabled(!1)
}