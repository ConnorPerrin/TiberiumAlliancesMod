...
function triggerAppear() {
    ...
    this.__GE();
    this.__Ok = !0;
    this.__SB.setEnabled(!1);
    this.__GU.setEnabled(!1);
    this.__Ol.setVisibility(n);
    this.__Os || (webfrontend.phe.cnc.Util.attachNetEvent(ClientLib.Data.MainData.GetInstance().get_Alliance(), v, ClientLib.Data.AllianceChange, this, this._onAllianceDetailChanged), this.__Os = !0)
}

function _onDisappearFake() {
    this.__Os && (webfrontend.phe.cnc.Util.detachNetEvent(ClientLib.Data.MainData.GetInstance().get_Alliance(), v, ClientLib.Data.AllianceChange, this, this._onAllianceDetailChanged), this.__Os = !1);
    this.__Pa()
}
...
function _onClickDisbandAlliance() {
    var n = qx.core.Init.getApplication();
    webfrontend.gui.MessageBox.messageBox({
        title: n.tr(e),
        text: n.tr(vt),
        textRich: !0,
        executeOk: this._onConfirmDisbandAlliance,
        callbackContext: this
    })
}

function _onConfirmDisbandAlliance() {
    ClientLib.Net.CommunicationManager.GetInstance().SendSimpleCommand(si, null, webfrontend.phe.cnc.Util.createEventDelegate(ClientLib.Net.CommandResult, this, this._onAllianceDisband), null)
}

function _onAllianceDisband(n, t) {
...
function _onClickApply() {
    ClientLib.Net.CommunicationManager.GetInstance().SendSimpleCommand(ai, {
        name: this.__SD.getValue().trim(),
        tag: this.__SE.getValue().trim()
    }, webfrontend.phe.cnc.Util.createEventDelegate(ClientLib.Net.CommandResult, this, this._onAllianceSetName), null)
}

function _onAllianceSetName(i, r) {
    if (this.__SB.setEnabled(!1), r != null)
        if (r != ClientLib.Base.EErrorCode.Success) {
            this.__Ol.setVisibility(t);
            switch (r) {
                case ClientLib.Base.EErrorCode.DuplicateName:
                    this.__Ol.setValue(this.tr(yi));
                    break;
                case ClientLib.Base.EErrorCode.InvalidName:
                    this.__Ol.setValue(this.tr(ti));
                    break;
                case ClientLib.Base.EErrorCode.InvalidShortName:
                    this.__Ol.setValue(this.tr(dt));
                    break;
                case ClientLib.Base.EErrorCode.DuplicateShortName:
                    this.__Ol.setValue(this.tr(fi));
                    break;
                default:
                    this.__Ol.setValue(this.tr(lt))
            }
        } else this.__Ol.setVisibility(n), this.__GU.setEnabled(!1)
}
...
function _onAllianceNameInput() {
    var r = ClientLib.Data.MainData.GetInstance().get_Alliance(),
        ...
        i = o,
        u, f;
    this.__SD.getValue().length < 6 ? i = this.tr(ft) : this.__SD.getValue().length > 20 ? i = this.tr(st) : this.__SD.getValue().match(new RegExp(l + ClientLib.Data.MainData.GetInstance().get_Server().get_AllowedLetters() + y)) ? i = this.tr(gt) : this.__SE.getValue().match(new RegExp(l + ClientLib.Data.MainData.GetInstance().get_Server().get_AllowedLetters() + y)) ? i = this.tr(et) : this.__SE.getValue().length < 3 && this.__Ok ? i = this.tr(ui) : this.__SE.getValue().length > 6 && this.__Ok ? i = this.tr(d) : this.__SD.getValue() == r.get_Name() && this.__SE.getValue() == r.get_Abbreviation() || (e = !0);
    this.__Ol.setValue(i);
    e ? (this.__SE.getValue().length > 0 && (u = r.get_LastRenameStep(), f = ClientLib.Data.MainData.GetInstance().get_Time(), this.__SB.setEnabled(u == 0 || u + f.get_StepsPerSecond() * 604800 <= f.GetServerStep())), this.__Ol.setVisibility(n)) : (this.__SB.setEnabled(!1), this.__Ol.setVisibility(i.length > 0 ? t : n));
    this.__GU.setEnabled(this.__SD.getValue() != r.get_Name() || this.__SE.getValue() != r.get_Abbreviation())
}
...
function _onClickReset() {
    var n = ClientLib.Data.MainData.GetInstance().get_Alliance();
    this.__SD.setValue(n.get_Name());
    this.__SE.setValue(n.get_Abbreviation());
    this.__GU.setEnabled(!1)
}

function _onClickEditDescription() {
    this.__LC.setVisibility(n);
    this.__OX.setVisibility(t);
    this.__OY.setVisibility(t);
    this.__SF.setVisibility(n);
    this.__SG.setVisibility(t);
    this.__OR = this.__SG.getValue()
}

function _onClickOK() {
    this.__LC.setVisibility(t);
    this.__OX.setVisibility(n);
    this.__OY.setVisibility(n);
    this.__Iz.setVisibility(t);
    this.__LD.setVisibility(t);
    this.__OT.setVisibility(n);
    this.__SF.setVisibility(t);
    this.__SG.setVisibility(n);
    ClientLib.Net.CommunicationManager.GetInstance().SendSimpleCommand(ei, {
        description: this.__SG.getValue()
    }, null, null)
}

function _onClickPreview() {
    this.__OX.setVisibility(n);
    this.__Iz.setVisibility(n);
    this.__OT.setVisibility(t);
    this.__LD.setVisibility(n);
    this.__SF.setVisibility(t);
    this.__SG.setVisibility(n);
    this.__OS = this.__yY.getValue();
    this.__yY.setValue(c + webfrontend.gui.util.BBCode.convertBBCode(this.__SG.getValue()) + u)
}

function _onClickCancelPreview() {
    this.__OX.setVisibility(t);
    this.__Iz.setVisibility(t);
    this.__OT.setVisibility(n);
    this.__LD.setVisibility(t);
...
function _onClickCancel() {
    ...
    this.__SF.setVisibility(t);
    this.__SG.setVisibility(n);
    this.__SG.setValue(this.__OR)
}

function _onKeyDownDescription() {
    var n = webfrontend.gui.alliance.PropertiesContainer.MaxDescriptionLength - this.form.__SG.getValue().length;
    this.lblCharCount.setValue(n.toString());
    n < 0 ? (this.btnOK.setEnabled(!1), this.lblAvailChars.setTextColor(h), this.lblCharCount.setTextColor(h)) : (this.btnOK.setEnabled(!0), this.lblAvailChars.setTextColor(s), this.lblCharCount.setTextColor(s))
}
