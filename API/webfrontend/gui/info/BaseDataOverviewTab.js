constructor: [Class webfrontend.gui.info.BaseDataOverviewTab]
open: function(){this.__Dq(true);var I=ClientLib.Data.MainData.GetInstance().get_Cities();phe.cnc.Util.attachNetEvent(I,t,ClientLib.Data.CitiesChange,this,this.__ym);phe.cnc.base.Timer.getInstance().addListener(d,this.__rB,this);}
close: function(){var J=ClientLib.Data.MainData.GetInstance().get_Cities();phe.cnc.Util.detachNetEvent(J,t,ClientLib.Data.CitiesChange,this,this.__ym);phe.cnc.base.Timer.getInstance().removeListener(d,this.__rB,this);}
__rB: function(){this.__Dq(false);}
__ym: function(){this.__Dq(true);}
__SR: function(name){var L=new qx.ui.container.Composite(new qx.ui.layout.HBox());var K=new qx.ui.basic.Label(name).set({textColor:u,font:r,marginBottom:10});L.add(new qx.ui.core.Spacer(),{flex:1});L.add(K);L.add(new qx.ui.core.Spacer(),{flex:1});return L;}
__SS: function(){this.__SB=new webfrontend.data.SimpleColFormattingDataModel();this.__Pe=new webfrontend.gui.widgets.CustomTable(this.__SB).set({statusBarVisible:false,columnVisibilityButtonVisible:false});}
__Dq: function(be){var bl=new Array();var bv=new Object();bv[B]=this.tr(y);var bd=new Object();bd[B]=this.tr(v);var bp=new Object();bp[B]=this.tr(E);var Y=new Object();Y[B]=this.tr(n);var N=new Object();N[B]=this.tr(g);var br=new Object();br[B]=this.tr(j);var X=new Object();X[B]=this.tr(A);var bb=new Object();bb[B]=this.tr(x);var M=new Object();M[B]=this.tr(C);var ba=new Object();ba[B]=this.tr(c);var S=new Object();S[B]=this.tr(z);var bo=new Object();bo[B]=this.tr(w);var P=new Object();P[B]=this.tr(k);var bk=ClientLib.Data.MainData.GetInstance().get_Cities().get_AllCities();var bh=new Array();bh.push(B);var bi=new Array();bi.push(this.tr(p));for(var T in bk.d){var O=ClientLib.Data.MainData.GetInstance().get_Time();var bt=O.GetServerStep();var V=bk.d[T];var W=V.get_Name();bi.push(W);bh.push(W);var bs=Math.floor(V.GetResourceCount(ClientLib.Base.EResourceType.RepairChargeBase));var R=phe.cnc.Util.getTimespanString(O.GetTimeSpan(bs));var bm=s;var bu=V.get_RepairOffenseResources();if(bu!=null){var bc=bu.get_RepairChargeOffense();if(!V.get_IsGhostMode()){var bn=bc!=null?bc.Type:0;var bs=Math.floor(V.GetResourceCount(bn));bm=phe.cnc.Util.getTimespanString(O.GetTimeSpan(bs));}else{if(bc==null){bm=phe.cnc.Util.getTimespanString(O.GetTimeSpan(0));}else{bm=phe.cnc.Util.getTimespanString(O.GetTimeSpan(bc.Base));}}}var bf=a;var bq=V.get_SupportAlertEndStep();if(bq>bt){var bj=V.get_DefenseAutoRepairStartStep();if(bj<=bt){bf=O.GetTimespanString(bq,bt);}}var U=s;var bg=V.get_SupportWeapon();if(bg!=null){U=o+ClientLib.File.FileManager.GetInstance().GetPhysicalPath(bg.i[D]+q)+e;}else{U=a;}bv[W]=(Math.floor(V.get_LvlBase()*100)/100).toFixed(2);bd[W]=V.GetBuildingSlotCount().toString()+F+V.GetBuildingSlotLimit().toString();bp[W]=V.GetBuildingsConditionInPercent()+h;Y[W]=R;N[W]=U;br[W]=(Math.floor(V.get_LvlOffense()*100)/100).toFixed(2);X[W]=V.get_TotalOffenseHeadCount().toString()+F+V.get_UnitLimitOffense().toString();bb[W]=V.GetOffenseConditionInPercent()>0?V.GetOffenseConditionInPercent()+h:a;M[W]=bm;ba[W]=(Math.floor(V.get_LvlDefense()*100)/100).toFixed(2);S[W]=V.get_TotalDefenseHeadCount().toString()+F+V.get_UnitLimitDefense().toString();bo[W]=V.GetDefenseConditionInPercent()>0?V.GetDefenseConditionInPercent()+h:a;P[W]=bf;}bl.push(bv);bl.push(bd);bl.push(bp);bl.push(Y);bl.push(N);bl.push(br);bl.push(new Object());bl.push(X);bl.push(bb);bl.push(M);bl.push(new Object());bl.push(ba);bl.push(S);bl.push(bo);bl.push(P);this.__SB.setColumns(bi,bh);this.__SB.setDataAsMapArray(bl,true,false);for(var i=0;i<bi.length;i++){this.__SB.setColumnSortable(i,false);}if(be){this.remove(this.__Pe);this.__Pe=new webfrontend.gui.widgets.CustomTable(this.__SB).set({statusBarVisible:false,columnVisibilityButtonVisible:false});this.add(this.__Pe,{flex:1});var Q=this.__Pe.getTableColumnModel();for(var i=1;i<=bk.c;i++){Q.setDataCellRenderer(i,new qx.ui.table.cellrenderer.Html());}Q.setColumnWidth(0,150);}}
