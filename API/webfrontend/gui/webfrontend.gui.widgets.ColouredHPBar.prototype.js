...
function setHPValues(u, f, e, s, a, v, d) {
    var nt;
    s || (s = 0);
    a || (a = 0);
    v || (v = 0);
    d || (d = 0);
    f == 0 ? this.__bcV.setVisibility(h) : f < .05 ? this.__bcV.setVisibility(r) : this.__bcV.setVisibility(r);
    var tt = Math.floor(this.__lF * u),
        it = Math.floor(this.__lF * f),
        rt = Math.floor(this.__lF * (e - f)),
        ut = Math.floor(this.__lF * s);
    this.__bcR.setWidth(tt);
    this.__bcS.setWidth(it);
    this.__bcT.setWidth(rt);
    this.__bcU.setWidth(ut);
    this.__bcS.setLayoutProperties({
        left: this.__bcY + tt
    });
    this.__bcT.setLayoutProperties({
        left: this.__bcY + tt + it
    });
    this.__bcU.setLayoutProperties({
        left: this.__bcY + tt + it + rt
    });
    this.__bcV.setLayoutProperties({
        left: this.__bcY + tt + it + rt + ut
    });
    this.__bcW.setLabel(this.tr(p, Math.floor(f * 100)));
    s > 0 && (nt = new qx.util.StringBuilder(1024), nt.add(this.tr(o)), nt.add(b + ClientLib.File.FileManager.GetInstance().GetPhysicalPath(k) + t), nt.add(a > 0 ? webfrontend.phe.cnc.gui.util.Numbers.calculateDisplayPercentValue(a / 100) : n, i), nt.add(y + ClientLib.File.FileManager.GetInstance().GetPhysicalPath(c) + t), nt.add(v > 0 ? webfrontend.phe.cnc.gui.util.Numbers.calculateDisplayPercentValue(v / 100) : n, i), nt.add(g + ClientLib.File.FileManager.GetInstance().GetPhysicalPath(l) + t), nt.add(d > 0 ? webfrontend.phe.cnc.gui.util.Numbers.calculateDisplayPercentValue(d / 100) : n, i), nt.add(w), this.__bcX.setLabel(nt.get()))
}