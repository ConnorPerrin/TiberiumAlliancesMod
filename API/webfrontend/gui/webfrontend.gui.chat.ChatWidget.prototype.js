...
function getHasAllianceForum anonymous() {
    this.getHasAllianceForum.$$install && this.getHasAllianceForum.$$install();
    return this.getHasAllianceForum.apply(this, arguments);
}
...
function setContentMinWidth(n) {
    for (var i = this.__GQ.getChildren(), t = 0; t < i.length; t++) i[t].ui.contcontent.setMinWidth(n)
}
...
function setTab(n) {
    var t = this.__bti(n);
    t && this.__GQ.setSelection([t])
}

function _onAppearFake() {
    var n = this.__GQ.getSelection()[0];
    n.ui.contcontent.add(this.__bsM, {
        flex: 1
    });
    this.__btr();
    this.focusInput();
    this.__bsW = !0;
    this.scrollToEnd(!0);
    this.fireDataEvent(d, this.__GQ.getSelection()[0].meta.flags)
}

function _onDisappearFake() {
    this.__bsM.getLayoutParent() && this.__bsM.getLayoutParent().remove(this.__bsM);
    this.__bsN.setValue(f);
    this.__bsW = !1
}

function syncMaxHeight(n) {
    var t = this.getBounds();
    t && this.__GQ.setMaxHeight(t.height);
    this.scrollToEnd(!n)
}
...
function send(n) {
    var g = this.__bsN.getValue(),
        l, r, a, s, h, o, u, tt;
    if (this.__bsN.setValue(f), n && webfrontend.gui.util.BBCode.stringIsVisible(n = n.trim())) {
        if (l = webfrontend.phe.cnc.Util.getConfigBoolean(ClientLib.Config.Main.CONFIG_SHOWGLOBALCHAT), r = this.__GQ.getSelection()[0], r.meta.flags == webfrontend.gui.chat.ChatWidget.channel.all && !l && n.charAt(0) != uo && !this.getHasAllianceForum()) {
            this.showMessage(i + this.tr(lf) + t, webfrontend.gui.chat.ChatWidget.sender.system, r.meta.flags);
            return
        }
        if (n == bu || n == oi || n == lu) {
            this.showMessage(i + this.tr(se) + t, webfrontend.gui.chat.ChatWidget.sender.system, r.meta.flags);
            return
        }
        if (this.__bsS = -1, this.__bsJ.unshift(n), this.__bsJ.length >= webfrontend.gui.chat.ChatWidget.sentbufsize && this.__bsJ.pop(), this.__bsT && n.indexOf(vt) == 0 && (this.showMessage(i + this.tr(oo) + t, webfrontend.gui.chat.ChatWidget.sender.system, r.meta.flags), this.__bsT = !1), n.charAt(0) != le) switch (r.meta.flags) {
            case webfrontend.gui.chat.ChatWidget.channel.all:
                n = l ? y + n : c + n;
                break;
            case webfrontend.gui.chat.ChatWidget.channel.alliance:
                n = c + n;
                break;
            case webfrontend.gui.chat.ChatWidget.channel.officers:
                n = ii + n;
                break;
            case webfrontend.gui.chat.ChatWidget.channel.whisper:
                this.__bsV ? n = v + this.__bsV + e + n : this.__bsU && (n = v + this.__bsU + e + n);
                break;
            case webfrontend.gui.chat.ChatWidget.channel.global:
                n = y + n
        } else if (n.indexOf(b) == 0) {
            if (!this.__bsU && !this.__bsV) {
                this.showMessage(i + this.tr(ir) + t, webfrontend.gui.chat.ChatWidget.sender.system, r.meta.flags);
                return
            }
            a = p + (this.__bsU ? this.__bsU : this.__bsV ? this.__bsV : f);
            a += n.indexOf(it) == 0 ? n.substr(2) : n.indexOf(ht) == 0 ? n.substr(6) : n.substr(2);
            n = a
        } if (s = new Date, this.__bta == null || this.__bta < s) {
            var nt = 0,
                w = 0,
                k = 0,
                d = new Date;
            for (d.setSeconds(d.getSeconds() - 5), h = new Date, h.setMinutes(h.getMinutes() - 1), o = new Date, o.setMinutes(o.getMinutes() - 5); this.__bsI.length > 0 && this.__bsI[0] < o;) this.__bsI.shift();
            for (u = 0; u < this.__bsI.length; u++) this.__bsI[u] >= d && nt++, this.__bsI[u] >= h && w++, this.__bsI[u] >= o && k++;
            nt >= 5 || w >= 15 || k >= 30 ? (this.__bta = new Date, k >= 30 ? this.__bta.setMinutes(this.__bta.getMinutes() + 5) : w >= 15 ? this.__bta.setMinutes(this.__bta.getMinutes() + 1) : this.__bta.setSeconds(this.__bta.getSeconds() + 5), this.__bsN.setValue(g), this.__btd()) : (n.substr(1, 6) != gf && n.substr(1, 8) != sf && n.substr(1, 4) != fe && n.substr(1, 6) != wu && n.substr(1, 9) != vu && n.substr(1, 6) != no && n.substr(1, 5) != lo && n.substr(1, 8) != tu && n.substr(1, 12) != hr && n.substr(1, 12) != ve && n.substr(1, 6) != fu && this.__bsI.push(s), n = qx.lang.Json.stringify(n), n = n.substring(1, n.length - 1), ClientLib.Data.MainData.GetInstance().get_Chat().AddMsg(qx.lang.String.stripScripts(qx.lang.String.stripTags(n))))
        } else tt = Math.floor((this.__bta - s) / 1e3), this.showMessage(this.__bto(tt), webfrontend.gui.chat.ChatWidget.sender.system, r.meta.flags), this.__bsN.setValue(g)
    }
}

function showMessage(n, t, i, r) {
    var o = webfrontend.phe.cnc.Util.getConfigBoolean(ClientLib.Config.Main.CONFIG_SHOWGLOBALCHAT),
        e, u, f;
    for ((t == webfrontend.gui.chat.ChatWidget.sender.system || r) && (o = !0), e = this.__GQ.getChildren(), u = 0; u < e.length; u++) f = e[u], (f.meta.flags & i) == 0 || i & webfrontend.gui.chat.ChatWidget.channel.global && f.meta.flags & webfrontend.gui.chat.ChatWidget.channel.all && !o || n && t && f.messages.push({
        message: n,
        sender: t,
        dispose: function() {}
    });
...
function isPlayerIgnored(n) {
    for (var i = qx.core.Init.getApplication().getChat().getIgnoreList(), r = !1, t = 0; t < i.length; t++)
        if (i[t].pn == n) {
            r = !0;
            break
        } return r
...
function ignoreUnignorePlayer(n, t) {
    var r = new qx.util.StringBuilder(1024),
        i;
    r.add(t ? ni : gt);
    r.add(e, n);
    i = qx.lang.Json.stringify(r.get());
    i = i.substring(1, i.length - 1);
    ClientLib.Data.MainData.GetInstance().get_Chat().AddMsg(qx.lang.String.stripScripts(qx.lang.String.stripTags(i)))
}

function scrollToEnd(n) {
    if (this.__bsW) {
        var t = this.__GQ.getSelection()[0];
        n && (t.autoscroll = n);
        t.autoscroll && (t.scrollcall || (t.scrollcall = new qx.util.DeferredCall(function() {
            this.scrollToY(this.getChildControl(ft).getMaximum())
        }, t.ui.scrcontent)), t.scrollcall.schedule())
    }
}