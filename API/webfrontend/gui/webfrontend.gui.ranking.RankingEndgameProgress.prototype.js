...
function open() {
    ...
    this.__bbI();
    webfrontend.phe.cnc.Util.attachNetEvent(ClientLib.Data.MainData.GetInstance().get_EndGame().GetCenter(), h, ClientLib.Data.EndGame.HubChanged, this, this.__bbI);
    webfrontend.phe.cnc.base.Timer.getInstance().addListener(s, this.__bbI, this)
}

function close() {
    this.__TN && (webfrontend.phe.cnc.base.Timer.getInstance().removeListenerById(this.__TN), this.__TN = null);
    this.__TB.setLocked(!0);
    webfrontend.phe.cnc.Util.detachNetEvent(ClientLib.Data.MainData.GetInstance().get_EndGame().GetCenter(), h, ClientLib.Data.EndGame.HubChanged, this, this.__bbI);
    webfrontend.phe.cnc.base.Timer.getInstance().removeListener(s, this.__bbI, this)
}

function searchRank(n) {
    !n || n.length <= 0 || this.__TX(this.__TB.getSortColumns()[0], n)
}
...
function searchName(n) {
    ...
    if (n && !(n.length < webfrontend.gui.ranking.RankingPlayerScore.searchValueMinLength)) {
        ...
        i >= 0 && this.__TX(this.__TB.getSortColumns()[i], n)
    }
}
...
function searchSelf() {
    switch (this.__TB.getViewType()) {
        case ClientLib.Data.Ranking.EViewType.Alliance:
            this.searchName(ClientLib.Data.MainData.GetInstance().get_Alliance().get_Name());
            break;
        case ClientLib.Data.Ranking.EViewType.Player:
        default:
            this.searchName(ClientLib.Data.MainData.GetInstance().get_Player().get_Name())
    }
    return
}