...
function deactivate() {
    var i, n;
    this.activated && (webfrontend.phe.cnc.base.Timer.getInstance().removeListener(b, this._onTick, this), i = ClientLib.Data.MainData.GetInstance().get_Cities(), webfrontend.phe.cnc.Util.detachNetEvent(i, t, ClientLib.Data.CitiesChange, this, this.__An), this.__bpD && webfrontend.phe.cnc.Util.detachNetEvent(this.__bpD, t, ClientLib.Data.CityChange, this, this._onUpdatePopupInfo), this.__bpE = null, this.__bpL = !1, this.activated = !1);
    this.__bpF = null;
    this.__bpH = null;
    this.__bpq.setSource(null);
    n = ClientLib.Vis.VisMain.GetInstance().get_ActiveMouseTool();
    n && (n.GetMouseToolType() == ClientLib.Vis.MouseTool.EMouseTool.MoveUnit || n.GetMouseToolType() == ClientLib.Vis.MouseTool.EMouseTool.MoveOffenseUnit) || ClientLib.Vis.VisMain.GetInstance().set_SelectedObject(null)
}
...
function activate(n) {
    if (this.activated) {
        if (this.__bpE == null || this.__bpG == null) return;
        this.__An()
    } else {
        ClientLib.Vis.VisMain.GetInstance().PlayUISound(lt);
        this.__bpE = n;
        this.__bpI = this.__bpE.get_CityEntityData().HasReachedMaxLevel();
        var i = ClientLib.Data.MainData.GetInstance().get_Cities();
        webfrontend.phe.cnc.Util.attachNetEvent(i, t, ClientLib.Data.CitiesChange, this, this.__An);
...
function _onUpdatePopupInfo() {
    if (this.__bpE && this.__bpD) {
        var u = this.__bpD.IsOwnBase(),
            o = this.__bpD.IsAllianceBase(),
            f = u ? e : l;
        this.__bpp.setVisibility(f);
        this.__bpB.setVisibility(f);
        var r = this.__bpE.get_UnitDetails().get_UnitGameData_Obj(),
            n = 0,
            t = i;
        if ((u || o) && r != null)
            if (r.pt == ClientLib.Base.EPlacementType.Offense) switch (r.mt) {
                case ClientLib.Base.EUnitMovementType.Feet:
                    n = ClientLib.Data.MainData.GetInstance().get_Alliance().get_POIInfantryBonus();
                    t = this.tr(c, n);
                    break;
                case ClientLib.Base.EUnitMovementType.Track:
                case ClientLib.Base.EUnitMovementType.Wheel:
                case ClientLib.Base.EUnitMovementType.Track_Wall:
                    n = ClientLib.Data.MainData.GetInstance().get_Alliance().get_POIVehicleBonus();
                    t = this.tr(c, n);
                    break;
                case ClientLib.Base.EUnitMovementType.Air:
                case ClientLib.Base.EUnitMovementType.Air2:
                    n = ClientLib.Data.MainData.GetInstance().get_Alliance().get_POIAirBonus();
                    t = this.tr(c, n)
            } else r.pt == ClientLib.Base.EPlacementType.Defense && (n = ClientLib.Data.MainData.GetInstance().get_Alliance().get_POIDefenseBonus(), t = this.tr(kt, n));
        n > 0 ? this.setTitle(this.__bpE.get_UnitName() + k + this.__bpE.get_UnitLevel() + g + t) : this.setTitle(this.__bpE.get_UnitName() + k + this.__bpE.get_UnitLevel() + a);
        this.__bpJ = this.__bpE.GetNumStepsUntilUpgradeAvailable();
        this.__bpK = ClientLib.Data.MainData.GetInstance().get_Time().GetServerStep();
        this.__bpM()
    }
}

function _onTick() {
    var n, p, b, i, c, l, a, d, h;
    if (this.__bpE && (this.__bpN(), n = !0, p = ClientLib.Data.MainData.GetInstance().get_Player().get_IsSubstituted(), p && (b = webfrontend.phe.cnc.Util.getConfigBoolean(ClientLib.Config.Main.CONFIG_SUBSTITUTIONALLOWUPGRADE), n = b, n || this.__bpC.setToolTipText(this.tr(yi))), n && (n = this.__bpE.CanExecuteCommand(ClientLib.Vis.Commands.UpgradeUnit)), this.__bpC.setEnabled(n), this.__bpt.exclude(), this.__bpu.exclude(), this.__bpr.exclude(), this.__bpA.exclude(), this.__bpH != null)) {
        var t = new qx.util.StringBuilder(1024),
            u = !1,
            k = this.__bpE.get_UnitDetails(),
            g = k.get_HitpointsPercent();
        this.__bps.updateResourceCosts(this.__bpH, this.__bpD, this.__bpI);
        n || (i = new qx.util.StringBuilder(1024), g < 1 && !this.__bpI && (i.add(o, webfrontend.theme.Color.colors[r], f), c = webfrontend.gui.UtilTech.getBuildingRepairableErrorStateText(k, this.__bpD), c.length > 0 && i.add(v, c, gt), i.add(v, this.tr(it), s), u = !0), this.__bpE.get_VisObjectType() == ClientLib.Vis.VisObject.EObjectType.DefenseUnitType ? (l = this.__bpD ? this.__bpD.get_CityBuildingsData().GetUniqueBuildingByTechName(ClientLib.Base.ETechName.Defense_HQ) : null, l != null && l.get_CurrentDamage() > 0 && (i.add(o, webfrontend.theme.Color.colors[r], f, this.tr(nt), s), u = !0)) : this.__bpE.get_VisObjectType() == ClientLib.Vis.VisObject.EObjectType.ArmyUnitType && (a = this.__bpD ? this.__bpD.get_CityBuildingsData().GetUniqueBuildingByTechName(ClientLib.Base.ETechName.Command_Center) : null, a != null && a.get_CurrentDamage() > 0 && (i.add(o, webfrontend.theme.Color.colors[r], f, this.tr(li), s), u = !0)), u && (this.__bpA.setValue(i.get()), this.__bpA.setVisibility(e)));
        u || this.__bpD.HasEnoughResources(this.__bpH) || this.__bpI || (d = ClientLib.Data.MainData.GetInstance().get_Time().GetServerStep(), h = d - this.__bpK, t.add(ui, this.tr(ei), y), this.__bpJ - h > 0 ? t.add(webfrontend.phe.cnc.Util.getTimespanString(ClientLib.Data.MainData.GetInstance().get_Time().GetTimeSpan(this.__bpJ - h), !0)) : this.__bpJ == 0 || this.__bpJ > 0 && this.__bpJ - h <= 0 ? t.add(this.tr(yt)) : t.add(this.tr(ut)), t.add(w), this.__bpJ - h > 0 ? (this.__bpt.setValue(t.get()), this.__bpt.show(), this.__bpu.exclude()) : (this.__bpu.setValue(t.get()), this.__bpt.exclude(), this.__bpu.show()));
        this.__bpr.show()
    }
}

function _onUpgrade() {
    (this.__bpC.setEnabled(!1), this.__bpE) && (this.__bpE.CanExecuteCommand(ClientLib.Vis.Commands.UpgradeUnit) && this.__bpE.ExecuteCommand(ClientLib.Vis.Commands.UpgradeUnit), this.onBtnClose())
}