constructor: [Class webfrontend.gui.alliance.MemberListPage]
appear: function(){if(!this.__MU){phe.cnc.Util.attachNetEvent(ClientLib.Data.MainData.GetInstance().get_Alliance(),G,ClientLib.Data.AllianceChange,this,this.__Sg);this.__MU=true;}this.__Sg(this.__SE.getMemberListSortOrderAsc(),this.__SE.getMemberListSortColumn());this.__qx=this.__SE.getSelected();this.__Pe.resetSelection();this.__SE.executeFilter();var bh=this.__SB.getRowCount();for(var i=0;i<bh;i++){var bi=this.__SB.getRowData(i);if(bi!==null&&this.__qx.indexOf(bi.Id)!==-1){this.__SB.setValue(0,i,true);}else{this.__SB.setValue(0,i,false);}}this.__SE.updateButtonStatus();}
disappear: function(){if(this.__MU){phe.cnc.Util.detachNetEvent(ClientLib.Data.MainData.GetInstance().get_Alliance(),G,ClientLib.Data.AllianceChange,this,this.__Sg);this.__MU=false;}this.__SE.setMemberListSortColumn(this.__SB.getSortColumnIndex());this.__SE.setMemberListSortOrderAsc(this.__SB.isSortAscending());}
getViewType: function(){return this.__SC;}
getTableModel: function(){return this.__SB;}
getTable: function(){return this.__Pe;}
resetMemberData: function(){var bj=this.__SF;this.__SB.setDataAsMapArray(bj,true,false);}
setOriginalData: function(bk){this.__SF=bk;}
getOriginalMemberData: function(){return this.__SF;}
filterMemberList: function(bp,bq,bn){this.__Pe.resetCellFocus();this.__Pe.resetSelection();var bm=this.__SF;if((bp===F||bp===null)&&(bq===null||bq<=0)&&(bn==null||bn==false)){this.__SB.setDataAsMapArray(bm,true,false);return;}if(bp.length>webfrontend.gui.ranking.RankingPlayerScore.searchValueMinLength){var bl=[];for(var j=0;j<bm.length;j++){var bo=bm[j];if(bo.Name.toLowerCase().indexOf(bp.toLowerCase())>=0){bl.push(bo);}}bm=bl;}if(bq!==null&&bq>0){var bl=[];for(var j=0;j<bm.length;j++){var bo=bm[j];if(parseInt(bo.Role)===bq){bl.push(bo);}}bm=bl;}if(bn==true){var bl=[];for(var j=0;j<bm.length;j++){var bo=bm[j];if(bo.ActiveState!=6){bl.push(bo);}}bm=bl;}this.__SB.setDataAsMapArray(bm,true,false);}
enableMemberSelectCheckbox: function(){if(ClientLib.Data.MainData.GetInstance().get_Alliance().get_CanPromoteLowerRoles()||ClientLib.Data.MainData.GetInstance().get_Alliance().get_CanKick()){this.__Pe.getTableColumnModel().setColumnVisible(0,true);}else{this.__Pe.getTableColumnModel().setColumnVisible(0,false);}}
_onClickTableCell: function(e){var br;switch(e.getColumn()){case 0:br=this.__SB.getRowData(e.getRow());br.b=!br.b;this.__SB.setValue(0,e.getRow(),br.b);this.__SE.setSelected(br.b,br.Id);this.__SE.updateButtonStatus();break;case 2:br=this.__SB.getRowData(e.getRow());webfrontend.gui.info.PlayerInfoWindow.getInstance().openWithPlayerId(br.Id);break;}}
__SG: function(){if(!this.__SD){this.__SB.setColumns([F,this.tr(z),this.tr(p),this.tr(V),this.tr(T),this.tr(C),this.tr(J),this.tr(x)],[y,S,Q,r,E,h,s,v]);}else{this.__SB.setSortMethods(6,this.__So);this.__SB.setColumns([F,this.tr(z),this.tr(p),this.tr(V),this.tr(T),this.tr(C),this.tr(U),this.tr(x)],[y,S,Q,r,E,h,d,v]);}this.__SB.setColFormat(2,n+webfrontend.gui.util.BBCode.clrLink+W,c);this.__SB.setSortMethods(3,function(a,b){return ClientLib.Data.MainData.GetInstance().get_Alliance().get_Roles().d[a.originalData.Role].SortOrder<ClientLib.Data.MainData.GetInstance().get_Alliance().get_Roles().d[b.originalData.Role].SortOrder?1:-1;});this.__SB.setSortMethods(7,this.__Sp);this.__SB.sortByColumn(2,true);this.__Pe=new webfrontend.gui.widgets.CustomTable(this.__SB).set({statusBarVisible:false,columnVisibilityButtonVisible:false,marginBottom:0,height:280});this.__Pe.addListener(q,this._onClickTableCell,this);var bs=this.__Pe.getTableColumnModel();bs.setDataCellRenderer(0,new qx.ui.table.cellrenderer.Boolean());bs.getDataCellRenderer(1).setUseAutoAlign(false);bs.setDataCellRenderer(2,new qx.ui.table.cellrenderer.Html());bs.getDataCellRenderer(3).setUseAutoAlign(false);bs.getDataCellRenderer(4).setUseAutoAlign(false);bs.getDataCellRenderer(5).setUseAutoAlign(false);bs.getDataCellRenderer(7).setUseAutoAlign(false);var bt=this.__Pe.getTableColumnModel();bt.setColumnWidth(0,24);bt.setColumnWidth(1,66);if(!this.__SD){bt.setColumnWidth(3,125);bt.setColumnWidth(4,60);bt.setColumnWidth(7,160);bt.setColumnWidth(6,100);}else{bt.setColumnWidth(3,135);bt.setColumnWidth(4,60);bt.setColumnWidth(7,100);bt.setColumnWidth(6,100);}bt.setColumnWidth(5,48);bt.setColumnWidth(6,66);bt.setHeaderCellRenderer(6,new qx.ui.table.headerrenderer.Default());if(!this.__SD){bt.getHeaderCellRenderer(6).setToolTip(this.tr(R));}this.add(this.__Pe,{flex:1});}
__SH: function(){this.__SB.setColumns([F,this.tr(z),this.tr(p),this.tr(C),this.tr(u),this.tr(N),this.tr(O),this.tr(w)],[y,S,Q,h,D,f,m,o]);this.__SB.setColFormat(2,n+webfrontend.gui.util.BBCode.clrLink+W,c);this.__SB.sortByColumn(2,true);this.__Pe=new webfrontend.gui.widgets.CustomTable(this.__SB).set({statusBarVisible:false,columnVisibilityButtonVisible:false,marginBottom:0,height:280});this.__Pe.addListener(q,this._onClickTableCell,this);var bu=this.__Pe.getTableColumnModel();bu.setDataCellRenderer(0,new qx.ui.table.cellrenderer.Boolean());bu.getDataCellRenderer(1).setUseAutoAlign(false);bu.setDataCellRenderer(2,new qx.ui.table.cellrenderer.Html());bu.getDataCellRenderer(3).setUseAutoAlign(false);bu.getDataCellRenderer(4).setUseAutoAlign(false);bu.getDataCellRenderer(5).setUseAutoAlign(false);bu.getDataCellRenderer(6).setUseAutoAlign(false);bu.getDataCellRenderer(7).setUseAutoAlign(false);var bv=this.__Pe.getTableColumnModel();bv.setColumnWidth(0,24);bv.setColumnWidth(1,66);bv.setColumnWidth(2,125);bv.setColumnWidth(3,70);bv.setColumnWidth(4,90);bv.setColumnWidth(5,90);bv.setColumnWidth(6,90);bv.setColumnWidth(7,90);this.add(this.__Pe,{flex:1});}
__Sg: function(by,bw){by=(typeof by===t)?null:by;bw=(typeof bw===t)?null:bw;var bz=ClientLib.Data.MainData.GetInstance().get_Alliance().get_MemberDataAsArray();var bB=webfrontend.gui.alliance.RosterPage.pages;if(bz===null){return;}switch(this.__SC){case bB.memberList:bz=this.__SI(bz);break;case bB.militaryStrength:bz=this.__SJ(bz);break;}this.__SF=bz;this.__SB.setDataAsMapArray(bz,true,false);var bA=(by===null||bw===null)?this.__SB.isSortAscending():by;var bx=(bw===null)?this.__SB.getSortColumnIndex():bw;if(bx===-1){bx=2;}this.__SB.sortByColumn(bx,bA);}
__SI: function(bD){for(var i=0;i<bD.length;i++){if(this.__qx.indexOf(bD[i].Id)<0){bD[i].b=false;}else{bD[i].b=true;}switch(bD[i].OnlineState){case ClientLib.Data.EMemberOnlineState.Offline:if(bD[i].ActiveState==6){bD[i].Rank=this.tr(K);}var bE=Date.now();if(bE-this.__RX*1000<=bD[i].LastSeen){bD[i].LastSeenString=this.tr(g,this.__RX/60);}else{bD[i].LastSeenString=this.tr(A)+bc+phe.cnc.Util.getDateTimeString(new Date(bD[i].LastSeen));}break;case ClientLib.Data.EMemberOnlineState.Online:bD[i].LastSeenString=this.tr(M);break;case ClientLib.Data.EMemberOnlineState.Away:bD[i].LastSeenString=this.tr(Y);break;case ClientLib.Data.EMemberOnlineState.Hidden:bD[i].LastSeenString=this.tr(bd);break;}bD[i].RoleNameReadable=webfrontend.gui.alliance.AllianceUtil.getAllianceRoleTranslation(bD[i].RoleName);var bC=ClientLib.Res.ResMain.GetInstance().GetPlayerTitleByLevel(bD[i].Faction,bD[i].Level);bD[i].Title=bC.dn;if(!ClientLib.Data.MainData.GetInstance().get_Server().get_IsSeasonServer()){if(typeof bD[i].HasControlHubCode==P){bD[i].HasControlHubCodeBool=bD[i].HasControlHubCode;}bD[i].HasControlHubCode=(bD[i].hasOwnProperty(k)&&bD[i].HasControlHubCodeBool)?this.tr(L):this.tr(l);}}return bD;}
__SJ: function(bF){return this.__SI(bF);}
