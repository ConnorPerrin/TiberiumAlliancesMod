constructor: [Class webfrontend.gui.alliance.RosterPage]
triggerAppear: function(){this.__Kv.schedule();}
setSelected: function(bN,bO){var bM=this.__qx.indexOf(bO);if(bN){if(bM===-1){this.__qx.push(bO);}}else{if(bM>-1){this.__qx.splice(bM,1);}}}
getSelected: function(){return this.__qx;}
updateButtonStatus: function(){this.__Si();}
executeFilter: function(){this._onFilter();}
setMemberListSortOrderAsc: function(bP){this.__Sb=bP;}
getMemberListSortOrderAsc: function(){return this.__Sb;}
setMemberListSortColumn: function(bQ){if([1,2].indexOf(bQ)!==-1){this.__Sa=bQ;}else{this.__Sa=null;}}
getMemberListSortColumn: function(){return this.__Sa;}
showSelectAllBtn: function(bR){if(bR){this.__Rt.show();}else{this.__Rt.hide();}}
_onAppearFake: function(){this.__Sf();this.__MF();this.__Sg();this.__Sj();this.__Sk();this.__Si();this.__Sh();this.__RU.setValue(P);this.__Se();ClientLib.Data.MainData.GetInstance().get_Alliance().RefreshMemberData();if(!this.__MU){phe.cnc.Util.attachNetEvent(ClientLib.Data.MainData.GetInstance().get_Alliance(),R,ClientLib.Data.AllianceChange,this,this._onAllianceDetailChanged);phe.cnc.Util.attachNetEvent(ClientLib.Data.MainData.GetInstance().get_Alliance(),bJ,ClientLib.Data.AllianceCanPromoteLowerRolesChange,this,this.__Sd);webfrontend.data.AllianceEvents.getInstance().addListener(bC,this.__Se,this);this.__MU=true;}}
_onDisappearFake: function(){if(this.__MU){phe.cnc.Util.detachNetEvent(ClientLib.Data.MainData.GetInstance().get_Alliance(),R,ClientLib.Data.AllianceChange,this,this._onAllianceDetailChanged);phe.cnc.Util.detachNetEvent(ClientLib.Data.MainData.GetInstance().get_Alliance(),bJ,ClientLib.Data.AllianceCanPromoteLowerRolesChange,this,this.__Sd);webfrontend.data.AllianceEvents.getInstance().removeListener(bC,this.__Se,this);this.__MU=false;}var bS=this.__Sc.getSelectables();for(var i in bS){var bT=bS[i];bT.getTable().resetSelection();}this.__qx=[];}
_onAllianceDetailChanged: function(){this.__MF();this.__Sg();this.__Sj();this.__Sk();this.__Si();}
_onSelectAll: function(e){this.__qx=[];var bU=this.__Sc.getSelectables();for(var i in bU){var bW=bU[i];var bV=bW.getTableModel().getData();var bX=bV.length;for(var j=0;j<bX;j++){bV[j]=bV[j].originalData;bV[j].b=e.getData();if(bV[j].b&&this.__qx.indexOf(bV[j].Id)<0){this.__qx.push(bV[j].Id);}}bW.getTableModel().setDataAsMapArray(bV,true);}this.__Si();}
_onClickHideOnlineStatusFromAllianceMembers: function(){ClientLib.Net.CommunicationManager.GetInstance().SendSimpleCommand(bH,{b:!this.__RP.getValue()},phe.cnc.Util.createEventDelegate(ClientLib.Net.CommandResult,this,this._onSetAllianceShowOnline),null);}
_onSetAllianceShowOnline: function(ca,bY){if(bY!=null&&bY!=0){webfrontend.gui.MessageBox.messageBox({title:this.tr(u),text:this.tr(H),buttons:1});this.__Sh();}}
_onClickGetAllianceMembersOnlineNotifications: function(){var cb=0;if(this.__RQ.getValue()){cb=1;}ClientLib.Config.Main.GetInstance().SetConfig(ClientLib.Config.Main.CONFIG_ONLINENOTIFICATIONS,cb);ClientLib.Config.Main.GetInstance().SaveToDB();}
_onClickManage: function(){if(this.__RS==null){this.__RS=new webfrontend.gui.alliance.MemberRightsWindow();}this.__RS.show();}
_onClickInvite: function(){var cc=qx.core.Init.getApplication().getCurrentMenuOverlay();cc.setTab(webfrontend.gui.alliance.AllianceOverlay.tabs.ETabOverviewPage);cc.getCurrentTab().setView(webfrontend.gui.alliance.OverviewPage.EViewInvitations);}
_onClickSetMemberRole: function(){var cd=-1;if(this.__RO.getSelection()[0]!=null){cd=this.__RO.getSelection()[0].getModel().id;}if(cd>=0){ClientLib.Net.CommunicationManager.GetInstance().SendSimpleCommand(bt,{memberIds:this.__qx,roleId:cd},phe.cnc.Util.createEventDelegate(ClientLib.Net.CommandResult,this,this._onAllianceSetMemberRole),null);}this.__Sf();}
_onAllianceSetMemberRole: function(cf,ce){if(ce!=null&&ce!=0){webfrontend.gui.MessageBox.messageBox({title:this.tr(J),text:this.tr(O),buttons:1});}}
_onClickDismiss: function(){var cj=ClientLib.Data.MainData.GetInstance().get_Server();var cg=cj.get_AllianceLeaveGracePeriodInMinutes();var ch=cg/60/24;var ci=qx.core.Init.getApplication();webfrontend.gui.MessageBox.messageBox({title:ci.tr(c),text:ci.tr(E,this.__qx.length,ch),textRich:true,executeOk:this._onConfirmDismiss,callbackContext:this});}
_onConfirmDismiss: function(){ClientLib.Net.CommunicationManager.GetInstance().SendSimpleCommand(K,{memberIds:this.__qx},phe.cnc.Util.createEventDelegate(ClientLib.Net.CommandResult,this,this._onAllianceKickMember),null);this.__Sf();}
_onAllianceKickMember: function(cm,cl){if(cl!=null){var ck=this.tr(bA);if(cl==5){ck=this.tr(o);}webfrontend.gui.MessageBox.messageBox({title:this.tr(W),text:ck,buttons:1});}}
_onClickLeaveAlliance: function(){var cn=ClientLib.Data.MainData.GetInstance().get_Alliance();if(cn.get_IsFirstLeader()){var cs=cn.get_FirstLeaders().l.length>1?this.tr(D):this.tr(F);var cp=qx.core.Init.getApplication();webfrontend.gui.MessageBox.messageBox({title:cp.tr(bx),text:cs,textRich:true,buttons:1});}else{var cr=ClientLib.Data.MainData.GetInstance().get_Server();var cq=cr.get_AllianceLeaveGracePeriodInMinutes();var co=cq/60/24;var cp=qx.core.Init.getApplication();webfrontend.gui.MessageBox.messageBox({title:cp.tr(bx),text:cp.tr(U,co),textRich:true,executeOk:this._onConfirmLeaveAlliance,callbackContext:this});}}
_onConfirmLeaveAlliance: function(){ClientLib.Net.CommunicationManager.GetInstance().SendSimpleCommand(G,null,phe.cnc.Util.createEventDelegate(ClientLib.Net.CommandResult,this,this._onAllianceLeave),null);}
_onAllianceLeave: function(cu,ct){if(ct!=null){if(ct!=0){webfrontend.gui.MessageBox.messageBox({title:this.tr(bx),text:this.tr(S),buttons:1});}}}
_onFilter: function(e){var cx=(this.__RU!==null)?this.__RU.getValue():null;var cv=(this.__RV!==null&&this.__RV.getSelection()!==null&&this.__RV.getSelection().length>0)?parseInt(this.__RV.getSelection()[0].getModel().id):0;var cw=(this.__RR!=null)?this.__RR.getValue():false;if(this.__Sc!==null){this.__Sc.getSelection()[0].filterMemberList(cx,cv,cw);}}
_onClickResetBtn: function(e){this.__RU.setValue(P);this.__RW=0;this.__RV.resetSelection();this.__RR.resetValue();this._onFilter();}
__Sd: function(cy){if(!cy){webfrontend.gui.alliance.AllianceUtil.openLostRightsPopup();}}
__Se: function(e){var cB=webfrontend.data.AllianceEvents.getInstance().getCanViewMilitary();var cA=this.__Sc.getSelectables();if(!cB){for(var i in cA){var cD=cA[i];if(cD.getViewType()===webfrontend.gui.alliance.RosterPage.pages.militaryStrength){this.__Sc.remove(cD);}}}else{for(var i in cA){cD=cA[i];if(cD.getViewType()===webfrontend.gui.alliance.RosterPage.pages.militaryStrength){return;}}var cC=webfrontend.gui.alliance.RosterPage.pages;var cz=new webfrontend.gui.alliance.MemberListPage(this.tr(k),cC.militaryStrength,this);cz.getChildControl(q).setToolTipText(this.tr(bE));this.__Sc.add(cz,{marginLeft:15,marginRight:15,marginTop:20});}}
__Sf: function(){this.__Rt.setValue(false);this.__qx=[];var cE=this.__Sc.getSelectables();var i,cG;for(i in cE){cG=cE[i];var cF=cG.getTableModel().getData();var cH=cF.length;for(var j=0;j<cH;j++){cF[j]=cF[j].originalData;cF[j].b=false;}cG.getTableModel().setDataAsMapArray(cF,true);}this.__Si();}
__MF: function(){var cJ=ClientLib.Data.MainData.GetInstance().get_Player().get_IsSubstituted();if(ClientLib.Data.MainData.GetInstance().get_Alliance().get_CanEditRights()){this.__RJ.setVisibility(bv);this.__RJ.setToolTipText(cJ?this.tr(by):null);this.__RJ.setEnabled(!cJ);}else{this.__RJ.setVisibility(m);}if((ClientLib.Data.MainData.GetInstance().get_Player().get_IsSubstituted()&&phe.cnc.Util.getConfigBoolean(ClientLib.Config.Main.CONFIG_SUBSTITUTIONALLOWALLYINVITES))||(!ClientLib.Data.MainData.GetInstance().get_Player().get_IsSubstituted()&&ClientLib.Data.MainData.GetInstance().get_Alliance().get_CanInvite())){this.__RK.setVisibility(bv);}else{this.__RK.setVisibility(m);}if(ClientLib.Data.MainData.GetInstance().get_Alliance().get_CanPromoteLowerRoles()){this.__RL.setVisibility(bv);this.__RL.setToolTipText(cJ?this.tr(d):null);this.__RL.setEnabled(!cJ);this.__RO.setVisibility(bv);this.__RO.setEnabled(!cJ);this.__RO.setToolTipText(cJ?this.tr(d):null);}else{this.__RL.setVisibility(m);this.__RO.setVisibility(m);}if(ClientLib.Data.MainData.GetInstance().get_Alliance().get_CanKick()){this.__RM.setVisibility(bv);this.__RM.setToolTipText(cJ?this.tr(Q):null);this.__RM.setEnabled(!cJ);}else{this.__RM.setVisibility(m);}if(ClientLib.Data.MainData.GetInstance().get_Alliance().get_IsAdmin()){this.__RN.setEnabled(false);this.__RN.setToolTipText(this.tr(D));}else{this.__RN.setEnabled(true);this.__RN.setToolTipText(P);}if(ClientLib.Data.MainData.GetInstance().get_Alliance().get_CanPromoteLowerRoles()||ClientLib.Data.MainData.GetInstance().get_Alliance().get_CanKick()){this.__Rt.show();}else{this.__Rt.exclude();}var cI=this.__Sc.getSelectables();for(var i in cI){cI[i].enableMemberSelectCheckbox();}}
__Sg: function(){var cK=ClientLib.Data.MainData.GetInstance().get_Alliance().get_MemberDataAsArray();if(cK==null){return;}this.__RT.setValue(this.tr(w)+bB+cK.length+I+ClientLib.Data.MainData.GetInstance().get_Alliance().get_AllianceMemberLimit());}
__Sh: function(){this.__RP.removeListener(g,this._onClickHideOnlineStatusFromAllianceMembers,this);this.__RQ.removeListener(g,this._onClickGetAllianceMembersOnlineNotifications,this);this.__RP.setValue(!ClientLib.Data.MainData.GetInstance().get_Alliance().get_ShowOnlineStatus());this.__RQ.setValue(parseInt(ClientLib.Config.Main.GetInstance().GetConfig(ClientLib.Config.Main.CONFIG_ONLINENOTIFICATIONS),10)==1);this.__RP.addListener(g,this._onClickHideOnlineStatusFromAllianceMembers,this);this.__RQ.addListener(g,this._onClickGetAllianceMembersOnlineNotifications,this);}
__Si: function(){if(ClientLib.Data.MainData.GetInstance().get_Player().get_IsSubstituted()){return;}var cL=ClientLib.Data.MainData.GetInstance().get_Player().get_Id();this.__RM.setEnabled(ClientLib.Data.MainData.GetInstance().get_Alliance().CanDismiss(this.__qx,cL));var cN=false;if(this.__RO.getSelection()[0]!=null){var cM=this.__RO.getSelection()[0].getModel().id;cN=ClientLib.Data.MainData.GetInstance().get_Alliance().CanSetMemberRole(this.__qx,cL,cM);}this.__RL.setEnabled(cN);this.__RO.setEnabled(cN);}
__Sj: function(){var cO=ClientLib.Data.MainData.GetInstance().get_Alliance();if(cO.get_Exists()){var cT=false;var cP=cO.get_Roles().d;var cU=ClientLib.Data.ERolesSortOrder.Member;var cS=this.__RO.getSelectables().length;if(cS!=0){var cV=this.__RO.getSelection()[0].getModel();var cR=cV.id;cU=cV.sortOrder;if(!cP.hasOwnProperty(cR)){cT=true;}}else{cT=true;}if(cT){this.__RO.removeAll();var cQ=null;for(var i in cP){if(!cO.getIsHigherRole(cP[i].SortOrder)&&!cO.get_IsAdmin()){continue;}var cW=new qx.ui.form.ListItem(webfrontend.gui.alliance.AllianceUtil.getAllianceRoleTranslation(cP[i].Name),null,{id:i.toString(),sortOrder:cP[i].SortOrder});this.__RO.add(cW);if(cP[i].SortOrder==cU){cQ=cW;}}if(cQ!=null){this.__RO.setSelection([cQ]);}}}}
__Sk: function(){var cX=ClientLib.Data.MainData.GetInstance().get_Alliance();if(cX.get_Exists()){this.__RV.removeListener(bs,this._onFilter,this);var dh=false;var cY=cX.get_Roles().d;var df=this.__RW;var dc=this.__RV.getSelectables().length;if(dc>0){var dg=this.__RV.getSelection()[0].getModel();var db=dg.id;df=dg.sortOrder;if(!cY.hasOwnProperty(db)){dh=true;}}else{dh=true;}if(dh){this.__RV.removeAll();var de=new qx.ui.form.ListItem(this.tr(X),null,{id:v,sortOrder:0});this.__RV.add(de);var da=de;for(var i in cY){var dd=new qx.ui.form.ListItem(webfrontend.gui.alliance.AllianceUtil.getAllianceRoleTranslation(cY[i].Name),null,{id:i.toString(),sortOrder:cY[i].SortOrder});this.__RV.add(dd);if(cY[i].SortOrder===df){da=dd;}}if(da!==null){this.__RV.setSelection([da]);}}this.__RV.addListener(bs,this._onFilter,this);}}
__Sl: function(){var dj=new webfrontend.gui.GroupBoxLarge(this.tr(bw));var dm=new qx.ui.container.Composite(new qx.ui.layout.VBox(0)).set({padding:8,marginBottom:10,height:300});dj.add(dm,{top:0,left:0,right:0});this.__Sc=new qx.ui.tabview.TabView();var dn=webfrontend.gui.alliance.RosterPage.pages;var dk=[dn.memberList,dn.militaryStrength];for(var i=0;i<dk.length;++i){var di=null;switch(dk[i]){case dn.memberList:di=new webfrontend.gui.alliance.MemberListPage(this.tr(T),dn.memberList,this);di.getChildControl(q).setToolTipText(this.tr(bE));break;case dn.militaryStrength:if(webfrontend.data.AllianceEvents.getInstance().getCanViewMilitary()){di=new webfrontend.gui.alliance.MemberListPage(this.tr(k),dn.militaryStrength,this);di.getChildControl(q).setToolTipText(this.tr(bE));}break;default:break;}if(di){this.__Sc.add(di,{marginLeft:15,marginRight:15,marginTop:20});}}var dl=new qx.ui.container.Composite(new qx.ui.layout.VBox(0));dl.add(this.__Sn());dm.add(dl);dm.add(this.__Sc);dm.add(this.__Sm());dm.add(this.__Dx());return dj;}
__Dx: function(){var dq=new qx.ui.container.Composite(new qx.ui.layout.Canvas());var dp=new qx.ui.container.Composite(new qx.ui.layout.VBox(3));this.__Rt=new qx.ui.form.CheckBox(this.tr(x)).set({marginBottom:10});this.__Rt.addListener(g,this._onSelectAll,this);this.__RP=new qx.ui.form.CheckBox(this.tr(y));this.__RP.addListener(g,this._onClickHideOnlineStatusFromAllianceMembers,this);this.__RQ=new qx.ui.form.CheckBox(this.tr(bu));this.__RQ.addListener(g,this._onClickGetAllianceMembersOnlineNotifications,this);var ds=new qx.ui.container.Composite(new qx.ui.layout.HBox(20)).set({marginBottom:10,marginTop:10});this.__RJ=new webfrontend.ui.SoundButton(this.tr(n));this.__RJ.addListener(s,this._onClickManage,this);this.__RK=new webfrontend.ui.SoundButton(this.tr(bF));this.__RK.addListener(s,this._onClickInvite,this);this.__RM=new webfrontend.ui.SoundButton(this.tr(p));this.__RM.setEnabled(false);this.__RM.addListener(s,this._onClickDismiss,this);this.__RN=new webfrontend.ui.SoundButton(this.tr(bx)).set({allowGrowX:false});this.__RN.addListener(s,this._onClickLeaveAlliance,this);ds.add(this.__RJ);ds.add(this.__RK);ds.add(this.__RM);dp.add(this.__Rt);dp.add(this.__RP);dp.add(this.__RQ);dp.add(ds);var dr=new qx.ui.container.Composite(new qx.ui.layout.HBox(5));this.__RO=new qx.ui.form.SelectBox().set({width:160});this.__RO.setVisibility(m);this.__RL=new webfrontend.ui.SoundButton(this.tr(f)).set({enabled:false});this.__RL.addListener(s,this._onClickSetMemberRole,this);this.__RL.setVisibility(m);dr.add(this.__RL);dr.add(this.__RO);var dt=new qx.ui.container.Composite(new qx.ui.layout.VBox(47));dt.add(this.__RN);dt.add(dr);dq.add(dp,{left:0,top:0});dq.add(dt,{right:0,top:0});return dq;}
__Sm: function(){var du=new qx.ui.container.Composite().set({layout:new qx.ui.layout.VBox(),decorator:B,padding:0,margin:0,marginBottom:10});var dv=new qx.ui.container.Composite().set({layout:new qx.ui.layout.HBox().set({alignY:bD,alignX:bI}),padding:6,margin:0});var dx=new qx.ui.container.Composite().set({layout:new qx.ui.layout.HBox().set({alignY:bD,alignX:M}),padding:6,margin:0});var dz=new qx.ui.basic.Image().set({source:N,height:24,width:29,scale:true});var dw=new qx.ui.form.TextField().set({placeholder:this.tr(l),maxLength:20,marginLeft:5,width:150,liveUpdate:true,maxHeight:24,height:24});this.__RU=dw;dw.addListener(g,this._onFilter,this);dv.add(dz);dv.add(dw);dv.add(new qx.ui.core.Spacer(12));this.__RV=new qx.ui.form.SelectBox().set({marginLeft:5,width:160,maxHeight:24,height:24});dv.add(new qx.ui.basic.Image().set({source:z,height:24,width:29,scale:true}));dv.add(this.__RV);dv.add(new qx.ui.core.Spacer(12));this.__RR=new qx.ui.form.CheckBox(this.tr(L)).set({height:24});this.__RR.addListener(g,this._onFilter,this);dv.add(this.__RR);dv.add(new qx.ui.core.Spacer(12));var dy=new webfrontend.ui.SoundButton(this.tr(bG)).set({maxHeight:24,height:24});dy.addListener(s,this._onClickResetBtn,this);dx.add(dy);du.add(dv);du.add(dx);return du;}
__Sn: function(){this.__RT=new qx.ui.basic.Label(P).set({textColor:A,font:t,decorator:Y,textAlign:C,allowGrowX:true});return this.__RT;}
__So: function(a,b){if(a.originalData.VeteranPointContribution>b.originalData.VeteranPointContribution){return -1;}return 1;}
__Sp: function(a,b){if(a.originalData.OnlineState!=ClientLib.Data.EMemberOnlineState.Offline&&b.originalData.OnlineState!=ClientLib.Data.EMemberOnlineState.Offline){if(a.originalData.OnlineState>b.originalData.OnlineState){return 1;}else{return -1;}}else{if(a.originalData.OnlineState!=ClientLib.Data.EMemberOnlineState.Offline){return -1;}else{if(b.originalData.OnlineState!=ClientLib.Data.EMemberOnlineState.Offline){return 1;}else{if(new Date(a.originalData.LastSeen)<new Date(b.originalData.LastSeen)){return 1;}else{return -1;}}}}}
getLabel: function(){return qx.core.Property.executeOptimizedGetter(this,clazz,name,bk);}
setLabel: function anonymous(value
) {
var prop=qx.core.Property;if(value===undefined)prop.error(this,2,"label","set",value);if(this.$$user_label===value)return value;var computed, old;if(this.$$runtime_label!==undefined){old=computed=this.$$runtime_label;this.$$user_label=value;}else if(this.$$user_label!==undefined){old=this.$$user_label;computed=this.$$user_label=value;}else if(this.$$useinit_label){old=this.$$init_label;delete this.$$useinit_label;computed=this.$$user_label=value;}else{computed=this.$$user_label=value;}if(old===computed)return value;if(old===undefined)old=this.$$init_label;this._applyLabel(computed, old, "label", "set");return value;
}
resetLabel: function(){return qx.core.Property.executeOptimizedSetter(this,clazz,name,ba);}
initLabel: function(ds){return qx.core.Property.executeOptimizedSetter(this,clazz,name,cz,arguments);}
setRuntimeLabel: function(du){return qx.core.Property.executeOptimizedSetter(this,clazz,name,bN,arguments);}
resetRuntimeLabel: function(){return qx.core.Property.executeOptimizedSetter(this,clazz,name,T);}
getIcon: function(){return qx.core.Property.executeOptimizedGetter(this,clazz,name,bk);}
setIcon: function anonymous(value
) {
var prop=qx.core.Property;if(value===undefined)prop.error(this,2,"icon","set",value);if(this.$$user_icon===value)return value;var computed, old;if(this.$$runtime_icon!==undefined){old=computed=this.$$runtime_icon;this.$$user_icon=value;}else if(this.$$user_icon!==undefined){old=this.$$user_icon;computed=this.$$user_icon=value;}else if(this.$$useinit_icon){old=this.$$init_icon;delete this.$$useinit_icon;computed=this.$$user_icon=value;}else{computed=this.$$user_icon=value;}if(old===computed)return value;if(old===undefined)old=this.$$init_icon;this._applyIcon(computed, old, "icon", "set");return value;
}
...
getShowCloseButton: function(){return qx.core.Property.executeOptimizedGetter(this,clazz,name,bk);}
setShowCloseButton: function(dr){return qx.core.Property.executeOptimizedSetter(this,clazz,name,e,arguments);}
resetShowCloseButton: function(){return qx.core.Property.executeOptimizedSetter(this,clazz,name,ba);}
initShowCloseButton: function(ds){return qx.core.Property.executeOptimizedSetter(this,clazz,name,cz,arguments);}
setRuntimeShowCloseButton: function(du){return qx.core.Property.executeOptimizedSetter(this,clazz,name,bN,arguments);}
resetRuntimeShowCloseButton: function(){return qx.core.Property.executeOptimizedSetter(this,clazz,name,T);}
toggleShowCloseButton: function anonymous(
) {
return this.setShowCloseButton(!this.getShowCloseButton())
}
isShowCloseButton: function anonymous(
) {
return this.getShowCloseButton()
}
_applyIcon: function(r,q){var p=this.getChildControl(k);if(r){p.setIcon(r);p._showChildControl(c);}else{p._excludeChildControl(c);}}
...
resetTextColor: function anonymous(value
) {
var prop=qx.core.Property;if(this.$$user_textColor===undefined)return;var inherit=prop.$$inherit;var computed, old=this.$$inherit_textColor;if(this.$$runtime_textColor!==undefined){old=computed=this.$$runtime_textColor;delete this.$$user_textColor;}else if(this.$$user_textColor!==undefined){delete this.$$user_textColor;if(this.$$runtime_textColor!==undefined)computed=this.$$runtime_textColor;if(this.$$theme_textColor!==undefined)computed=this.$$theme_textColor;else if(this.$$init_textColor!==undefined){computed=this.$$init_textColor;this.$$useinit_textColor=true;}}else if(this.$$theme_textColor!==undefined){}else if(this.$$useinit_textColor){}if(computed===undefined||computed===inherit){var pa=this.getLayoutParent();if(pa)computed=pa.$$inherit_textColor;if((computed===undefined||computed===inherit)&&this.$$init_textColor!==undefined&&this.$$init_textColor!==inherit){computed=this.$$init_textColor;this.$$useinit_textColor=true;}else{delete this.$$useinit_textColor;}}if(old===computed)return value;if(computed===inherit){computed=undefined;delete this.$$inherit_textColor;}else if(computed===undefined)delete this.$$inherit_textColor;else this.$$inherit_textColor=computed;var backup=computed;if(old===undefined)old=null;if(computed===undefined||computed==inherit)computed=null;this._applyTextColor(computed, old, "textColor", "reset");var reg=qx.event.Registration;if(reg.hasListener(this, 'changeTextColor')){reg.fireEvent(this, 'changeTextColor', qx.event.type.Data, [computed, old])}var a=this._getChildren();if(a)for(var i=0,l=a.length;i<l;i++){if(a[i].refreshTextColor)a[i].refreshTextColor(backup);}
}
...
getContextMenu: function anonymous(value
) {
if(this.$$runtime_contextMenu!==undefined)return this.$$runtime_contextMenu;if(this.$$user_contextMenu!==undefined)return this.$$user_contextMenu;else return null;
}
